...
class NotificationService {
  async getAllNotificationByOrganizer(organiserId) {
            ...
            if (result) {
              ...
              if (!existing) {
                const notification = {
                  ...
                  logoUrl: elm.logoUrl,
                  msg:
                    "Reminder: Your auction for tournament " +
                    elm.name +
                    " is starting soon.",
                  type: "auction_reminder",
                };
  ...
  async notificationViewByOrganizer(organiserId, notificationId) {
    return new Promise(async (resolve, reject) => {
      try {
  ...
  async sendNotificationToOrganizer(data) {
    return new Promise(async (resolve, reject) => {
      try {
        const getTournament = await Tournament.findById({
        ...
        const io = getIO();
        io.to(`${result?.organiserId}-organizer-notification`).emit(
          "notification-sent",
          { result }
        );
        resolve();
  ...
  async notificationAllReadByOrganizer(organiserId) {
    return new Promise(async (resolve, reject) => {
      console.log(
        "Marking all notifications as read for organiser:",
        organiserId
      );
      try {
  ...
  async getAllNotificationsOfUser(userId, page = 1, limit = 10) {
    return new Promise(async (resolve, reject) => {
      try {
  ...
  async markNotificationViewed(userId, notificationId) {
    return new Promise(async (resolve, reject) => {
      try {
        const result = await Notification.findOneAndUpdate(
  ...
  async markNotificationAllRead(userId) {
    return new Promise(async (resolve, reject) => {
      try {
        const result = await Notification.updateMany(
